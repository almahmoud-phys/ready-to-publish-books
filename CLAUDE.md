...
5. **Context discipline** — each skill's frontmatter declares what it may and may not read. Honor it strictly (see `.agents/rules/` + skill frontmatter).
6. **Gates live in rules, not sessions** — thresholds change only via edits to `.agents/rules/quality-gates.md`, never mid-session (Drifting Goals tripwire).
